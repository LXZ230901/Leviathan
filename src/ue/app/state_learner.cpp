...
void UeStateLearner::execute_command(std::string msg) 
{
    ...
    else
    {
        ...
        case MsgType::corruptIe:{
            enableFuzzing = false;
            notify_response("OK");
            std::cout << "corruptIe" << std::endl;
            // get size of binary message
            size_t size = 1000;
            if (index != std::string::npos)
                size = std::stoi(sub_str);
            // read corruptIe data: <msgName>:<ieIndex>:<hexBytes>
            ...
            {
                notify_response("invalid corruptIe format, expected msgName:ieIndex:hexBytes");
                enableFuzzing = true;
                break;
            }
            std::string msgName = rawStr.substr(0, c1);
            int ieIndex = std::stoi(rawStr.substr(c1 + 1, c2 - c1 - 1));
            OctetString corruptBytes = OctetString::FromHex(rawStr.substr(c2 + 1));
            std::cout << "corruptIe: msg=" << msgName << " ieIndex=" << ieIndex << " bytes=" << corruptBytes.toHexString() << std::endl;
            ...
            // dispatch to the stored message and call onCorrupt
            switch (targetType)
            {
            case MsgType::registrationRequestIMSI:
                registrationRequestIMSI.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(registrationRequestIMSI);
                break;
            case MsgType::registrationRequestGUTI:
                registrationRequestGUTI.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(registrationRequestGUTI);
                break;
            case MsgType::registrationComplete:{
                auto msg = nas::RegistrationComplete{};
                msg.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(msg);
                break;}
            case MsgType::deregistrationRequest:
                deregistrationRequest.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(deregistrationRequest);
                break;
            case MsgType::serviceRequest:
                serviceRequest.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendServiceRequest(EServiceReqCause::IDLE_UPLINK_SIGNAL_PENDING);
                mm->sendNasMessage(serviceRequest);
                break;
            case MsgType::securityModeReject:
                securityModeReject.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(securityModeReject);
                break;
            case MsgType::authenticationResponse:
                authenticationResponse.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(authenticationResponse);
                break;
            case MsgType::authenticationFailure:
                authenticationFailure.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(authenticationFailure);
                break;
            case MsgType::securityModeComplete:
                securityModeComplete.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(securityModeComplete);
                break;
            case MsgType::identityResponse:
                identityResponse.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(identityResponse);
                break;
            case MsgType::configurationUpdateComplete:{
                ...
                break;}
            case MsgType::gmmStatus:
                gmmStatus.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(gmmStatus);
                break;
            case MsgType::ulNasTransport:
                ulNasTransport.onCorrupt(mutator, ieIndex, corruptBytes);
                mm->sendNasMessage(ulNasTransport);
                break;
            default:
                notify_response("message type not supported for corruptIe");
                break;
            }
