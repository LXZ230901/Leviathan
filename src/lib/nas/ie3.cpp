...
void IE5gMmCause::Corrupt(IE5gMmCause &ie, const OctetString &bytes)
{
    ie.value = (EMmCause)(bytes.length() > 0 ? (int)bytes.data()[0] : 0);
}
...
void IE5gsTrackingAreaIdentity::Corrupt(IE5gsTrackingAreaIdentity &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        auto decoded = IE5gsTrackingAreaIdentity::Decode(view);
        ie.mcc = decoded.mcc;
        ie.mnc = decoded.mnc;
        ie.isLongMnc = decoded.isLongMnc;
        ie.trackingAreaCode = decoded.trackingAreaCode;
    } catch (...) {}
}
...
void IEAuthenticationParameterRand::Corrupt(IEAuthenticationParameterRand &ie, const OctetString &bytes)
{
    ie.value = bytes.copy();
}
...
void IEEpsNasSecurityAlgorithms::Corrupt(IEEpsNasSecurityAlgorithms &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.integrity = (EEpsTypeOfIntegrityProtectionAlgorithm)(b & 0x7);
    ie.ciphering = (EEpsTypeOfCipheringAlgorithm)((b >> 4) & 0x7);
}
...
void IEGprsTimer::Corrupt(IEGprsTimer &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.timerValue = b & 0x1F;
    ie.timerValueUnit = (EGprsTimerValueUnit)((b >> 5) & 0x7);
}
...
void IEIntegrityProtectionMaximumDataRate::Corrupt(IEIntegrityProtectionMaximumDataRate &ie, const OctetString &bytes)
{
    ie.maxRateUplink = (EMaximumDataRatePerUeForUserPlaneIntegrityProtectionForUplink)(bytes.length() > 0 ? (int)bytes.data()[0] : 0);
    ie.maxRateDownlink = (EMaximumDataRatePerUeForUserPlaneIntegrityProtectionForDownlink)(bytes.length() > 1 ? (int)bytes.data()[1] : 0);
}
...
void IEMaximumNumberOfSupportedPacketFilters::Corrupt(IEMaximumNumberOfSupportedPacketFilters &ie, const OctetString &bytes)
{
    int v = 0;
    if (bytes.length() >= 2) v = ((int)bytes.data()[0] << 8) | (int)bytes.data()[1];
    else if (bytes.length() == 1) v = (int)bytes.data()[0];
    ie.value = v & 0x7FF;
}
...
void IEN1ModeToS1ModeNasTransparentContainer::Corrupt(IEN1ModeToS1ModeNasTransparentContainer &ie, const OctetString &bytes)
{
    ie.sequenceNumber = bytes.length() > 0 ? bytes.data()[0] : 0;
}
...
void IENasSecurityAlgorithms::Corrupt(IENasSecurityAlgorithms &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.integrity = (ETypeOfIntegrityProtectionAlgorithm)(b & 0xF);
    ie.ciphering = (ETypeOfCipheringAlgorithm)((b >> 4) & 0xF);
}
...
void IEPduSessionIdentity2::Corrupt(IEPduSessionIdentity2 &ie, const OctetString &bytes)
{
...
void IETimeZone::Corrupt(IETimeZone &ie, const OctetString &bytes)
{
    ie.value = bytes.length() > 0 ? bytes.data()[0] : 0;
}
...
void IETimeZoneAndTime::Corrupt(IETimeZoneAndTime &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie.time = VTime::Decode(view);
        if (bytes.length() >= 7)
            ie.timezone = view.read();
    } catch (...) {}
}
...
void IE5gSmCause::Corrupt(IE5gSmCause &ie, const OctetString &bytes)
{
    ie.value = (ESmCause)(bytes.length() > 0 ? (int)bytes.data()[0] : 0);
}
