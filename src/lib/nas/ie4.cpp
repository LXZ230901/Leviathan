...
void IEDaylightSavingTime::Corrupt(IEDaylightSavingTime &ie, const OctetString &bytes)
{
    ie.value = (EDaylightSavingTime)(bytes.length() > 0 ? (int)bytes.data()[0] : 0);
}
...
void IEPduSessionReactivationResult::Corrupt(IEPduSessionReactivationResult &ie, const OctetString &bytes)
{
    for (int i = 0; i < 8 && bytes.length() > 0; i++)
        ie.psi[i] = (bytes.data()[0] >> i) & 1;
    for (int i = 0; i < 8 && bytes.length() > 1; i++)
        ie.psi[8 + i] = (bytes.data()[1] >> i) & 1;
}
...
void IEPduAddress::Corrupt(IEPduAddress &ie, const OctetString &bytes)
{
    ie.sessionType = (EPduSessionType)(bytes.length() > 0 ? (bytes.data()[0] & 0x7) : 0);
    ie.pduAddressInformation = bytes.length() > 1 ? bytes.subCopy(1) : OctetString{};
}
...
void IESNssai::Corrupt(IESNssai &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        auto decoded = IESNssai::Decode(view, bytes.length());
        ie = decoded;
    } catch (...) {
        if (bytes.length() > 0) ie.sst = bytes.data()[0];
    }
}
...
void IEAdditional5gSecurityInformation::Corrupt(IEAdditional5gSecurityInformation &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.hdp = (EHorizontalDerivationParameter)(b & 1);
    ie.rinmr = (ERetransmissionOfInitialNasMessageRequest)((b >> 1) & 1);
}
...
void IES1UeNetworkCapability::Corrupt(IES1UeNetworkCapability &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IES1UeNetworkCapability::Decode(view, bytes.length());
    } catch (...) {}
}
...
void IEGprsTimer3::Corrupt(IEGprsTimer3 &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.timerValue = b & 0x1F;
    ie.unit = (EGprsTimerValueUnit3)((b >> 5) & 0x7);
}
...
void IEAuthenticationFailureParameter::Corrupt(IEAuthenticationFailureParameter &ie, const OctetString &bytes)
{
    ie.rawData = bytes.copy();
}
...
void IEAbba::Corrupt(IEAbba &ie, const OctetString &bytes)
{
    ie.rawData = bytes.copy();
}
...
void IES1ModeToN1ModeNasTransparentContainer::Corrupt(IES1ModeToN1ModeNasTransparentContainer &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IES1ModeToN1ModeNasTransparentContainer::Decode(view, bytes.length());
    } catch (...) {}
}
...
void IEGprsTimer2::Corrupt(IEGprsTimer2 &ie, const OctetString &bytes)
{
    ie.value = bytes.length() > 0 ? bytes.data()[0] : 0;
}
...
void IE5gSmCapability::Corrupt(IE5gSmCapability &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.rqos = (EReflectiveQoS)(b & 1);
    ie.mh6pdu = (EMultiHomedIPv6PduSession)((b >> 1) & 1);
}
...
void IEUeSecurityCapability::Corrupt(IEUeSecurityCapability &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IEUeSecurityCapability::Decode(view, bytes.length());
    } catch (...) {}
}
...
void IESessionAmbr::Corrupt(IESessionAmbr &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IESessionAmbr::Decode(view, bytes.length());
    } catch (...) {}
}
...
void IEAuthenticationParameterAutn::Corrupt(IEAuthenticationParameterAutn &ie, const OctetString &bytes)
{
    ie.value = bytes.copy();
}
...
void IE5gsUpdateType::Corrupt(IE5gsUpdateType &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.smsRequested = (ESmsRequested)(b & 1);
    ie.ngRanRcu = (ENgRanRadioCapabilityUpdate)((b >> 1) & 1);
}
...
void IEUplinkDataStatus::Corrupt(IEUplinkDataStatus &ie, const OctetString &bytes)
{
    for (int i = 0; i < 8 && bytes.length() > 0; i++)
        ie.psi[i] = (bytes.data()[0] >> i) & 1;
    for (int i = 0; i < 8 && bytes.length() > 1; i++)
...
void IEAdditionalInformation::Corrupt(IEAdditionalInformation &ie, const OctetString &bytes)
{
    ie.rawData = bytes.copy();
}
...
void IEAuthenticationResponseParameter::Corrupt(IEAuthenticationResponseParameter &ie, const OctetString &bytes)
{
    ie.rawData = bytes.copy();
}
...
void IEUeStatus::Corrupt(IEUeStatus &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.s1ModeReg = (EEmmRegistrationStatus)(b & 1);
    ie.n1ModeReg = (E5gMmRegistrationStatus)((b >> 1) & 1);
}
...
void IE5gsRegistrationResult::Corrupt(IE5gsRegistrationResult &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.registrationResult = (E5gsRegistrationResult)(b & 0x7);
    ie.smsOverNasAllowed = (ESmsOverNasTransportAllowed)((b >> 3) & 1);
}
...
void IENetworkName::Corrupt(IENetworkName &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        if (bytes.length() >= 1) {
            int flags = view.readI();
            ie.numOfSpareBits = flags & 0x7;
            ie.addCi = (EAddCountryInitials)((flags >> 3) & 1);
            ie.codingScheme = (ECodingScheme)((flags >> 4) & 0x7);
            ie.textString = view.readOctetString(bytes.length() - 1);
        }
    } catch (...) {}
}
...
void IEUesUsageSetting::Corrupt(IEUesUsageSetting &ie, const OctetString &bytes)
{
    ie.value = (EUesUsageSetting)(bytes.length() > 0 ? (int)bytes.data()[0] : 0);
}
...
void IEIntraN1ModeNasTransparentContainer::Corrupt(IEIntraN1ModeNasTransparentContainer &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IEIntraN1ModeNasTransparentContainer::Decode(view, bytes.length());
    } catch (...) {}
}
...
void IEAllowedPduSessionStatus::Corrupt(IEAllowedPduSessionStatus &ie, const OctetString &bytes)
{
    for (int i = 0; i < 8 && bytes.length() > 0; i++)
        ie.psi[i] = (bytes.data()[0] >> i) & 1;
    for (int i = 0; i < 8 && bytes.length() > 1; i++)
...
void IEPduSessionStatus::Corrupt(IEPduSessionStatus &ie, const OctetString &bytes)
{
    for (int i = 0; i < 8 && bytes.length() > 0; i++)
        ie.psi[i] = (bytes.data()[0] >> i) & 1;
    for (int i = 0; i < 8 && bytes.length() > 1; i++)
...
void IE5gsDrxParameters::Corrupt(IE5gsDrxParameters &ie, const OctetString &bytes)
{
    ie.value = (EDrxValue)(bytes.length() > 0 ? (int)bytes.data()[0] : 0);
}
...
void IE5gMmCapability::Corrupt(IE5gMmCapability &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.s1Mode = (EEpcNasSupported)(b & 1);
    ie.hoAttach = (EHandoverAttachSupported)((b >> 1) & 1);
    ie.lpp = (ELtePositioningProtocolCapability)((b >> 2) & 1);
}
...
void IESmPduDnRequestContainer::Corrupt(IESmPduDnRequestContainer &ie, const OctetString &bytes)
{
    ie.dnSpecificIdentity = bytes.copy();
}
...
void IE5gsNetworkFeatureSupport::Corrupt(IE5gsNetworkFeatureSupport &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IE5gsNetworkFeatureSupport::Decode(view, bytes.length());
    } catch (...) {}
}
...
void IEDnn::Corrupt(IEDnn &ie, const OctetString &bytes)
{
    ie.apn = bytes.copy();
}
...
void IENssai::Corrupt(IENssai &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListIe(view, bytes.length(), ie.sNssais);
    } catch (...) {
        ie.sNssais.clear();
    }
}
...
void IEPlmnList::Corrupt(IEPlmnList &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListVal(view, bytes.length(), ie.plmns);
    } catch (...) {
        ie.plmns.clear();
    }
}
...
void IEEmergencyNumberList::Corrupt(IEEmergencyNumberList &ie, const OctetString &bytes)
{
    ie.rawData = bytes.copy();
}
...
void IERejectedNssai::Corrupt(IERejectedNssai &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListVal(view, bytes.length(), ie.list);
    } catch (...) {
        ie.list.clear();
    }
}
...
void IEServiceAreaList::Corrupt(IEServiceAreaList &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListVal(view, bytes.length(), ie.list);
...
void IE5gsTrackingAreaIdentityList::Corrupt(IE5gsTrackingAreaIdentityList &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListVal(view, bytes.length(), ie.list);
