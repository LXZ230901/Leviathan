...
//======================================================================================================
//                                  CORRUPT FUNCTIONS FOR FUZZING
//======================================================================================================

template <typename T, typename U>
static inline void CorruptIe1(T &big, U &little, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement1, T>::value);
    int val = bytes.length() > 0 ? (bytes.data()[0] & 0xF) : 0;
    T::Corrupt(big, val);
    U::Corrupt(little, val);
}

template <typename T>
static inline void CorruptIe1(int big, T &little, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement1, T>::value);
    int val = bytes.length() > 0 ? (bytes.data()[0] & 0xF) : 0;
    T::Corrupt(little, val);
}

template <typename T>
static inline void CorruptIe1(T &big, int little, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement1, T>::value);
    int val = bytes.length() > 0 ? (bytes.data()[0] & 0xF) : 0;
    T::Corrupt(big, val);
...
template <typename T>
static inline void CorruptIe2(T &ie, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement2, T>::value);
    // nothing to corrupt for type-2 information elements
}

template <typename T>
static inline void CorruptIe3(T &ie, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement3, T>::value);
    T::Corrupt(ie, bytes);
}
...
template <typename T>
static inline void CorruptIe4(T &ie, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement4, T>::value);
    T::Corrupt(ie, bytes);
}
...
template <typename T>
static inline void CorruptIe6(T &ie, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement6, T>::value);
    T::Corrupt(ie, bytes);
}
...
template <typename T>
static inline void Corrupt2346(T &ie, const OctetString &bytes)
{
    static_assert(std::is_base_of<InformationElement2, T>::value || std::is_base_of<InformationElement3, T>::value ||
                  std::is_base_of<InformationElement4, T>::value || std::is_base_of<InformationElement6, T>::value);
    if constexpr (std::is_base_of<InformationElement2, T>::value)
        CorruptIe2<T>(ie, bytes);
    if constexpr (std::is_base_of<InformationElement3, T>::value)
        CorruptIe3<T>(ie, bytes);
    if constexpr (std::is_base_of<InformationElement4, T>::value)
        CorruptIe4<T>(ie, bytes);
    if constexpr (std::is_base_of<InformationElement6, T>::value)
        CorruptIe6<T>(ie, bytes);
}
} // namespace nas