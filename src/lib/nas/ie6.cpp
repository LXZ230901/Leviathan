...
void IEQoSFlowDescriptions::Corrupt(IEQoSFlowDescriptions &ie, const OctetString &bytes)
{
    try {
...
void IEPayloadContainer::Corrupt(IEPayloadContainer &ie, const OctetString &bytes)
{
    ie.data = bytes.copy();
}
...
void IEExtendedEmergencyNumberList::Corrupt(IEExtendedEmergencyNumberList &ie, const OctetString &bytes)
{
    ie.data = bytes.copy();
}
...
void IEEpsNasMessageContainer::Corrupt(IEEpsNasMessageContainer &ie, const OctetString &bytes)
{
    ie.data = bytes.copy();
}
...
void IENasMessageContainer::Corrupt(IENasMessageContainer &ie, const OctetString &bytes)
{
    ie.data = bytes.copy();
}
...
void IEExtendedProtocolConfigurationOptions::Corrupt(IEExtendedProtocolConfigurationOptions &ie, const OctetString &bytes)
{
    int b = bytes.length() > 0 ? bytes.data()[0] : 0;
    ie.configurationProtocol = (EConfigurationProtocol)(b & 0x7);
    ie.extension = (b >> 7) & 1;
    ie.options = bytes.length() > 1 ? bytes.subCopy(1) : OctetString{};
}
...
void IEPduSessionReactivationResultErrorCause::Corrupt(IEPduSessionReactivationResultErrorCause &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListVal(view, bytes.length(), ie.values);
    } catch (...) {}
}
...
void IELadnIndication::Corrupt(IELadnIndication &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListIe(view, bytes.length(), ie.values);
    } catch (...) {}
}
...
void IELadnInformation::Corrupt(IELadnInformation &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListVal(view, bytes.length(), ie.ladnList);
    } catch (...) {}
}
...
void IEOperatorDefinedAccessCategoryDefinitions::Corrupt(IEOperatorDefinedAccessCategoryDefinitions &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        DecodeListVal(view, bytes.length(), ie.values);
...
void IEMappedEpsBearerContexts::Corrupt(IEMappedEpsBearerContexts &ie, const OctetString &bytes)
{
    ie.data = bytes.copy();
}
...
void IEQoSRules::Corrupt(IEQoSRules &ie, const OctetString &bytes)
{
    ie.data = bytes.copy();
}
...
void IESorTransparentContainer::Corrupt(IESorTransparentContainer &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IESorTransparentContainer::Decode(view, bytes.length());
    } catch (...) {}
}
...
void IE5gsMobileIdentity::Corrupt(IE5gsMobileIdentity &ie, const OctetString &bytes)
{
    try {
        OctetView view(bytes);
        ie = IE5gsMobileIdentity::Decode(view, bytes.length());
    } catch (...) {
        ie.type = (EIdentityType)(bytes.length() > 0 ? (bytes.data()[0] & 0x7) : 0);
    }
}
...
void IEEapMessage::Corrupt(IEEapMessage &ie, const OctetString &bytes)
{
    // Not corrupting EAP messages for now
}
